...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/unraid-mcp-server/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[unraid-mcp-server] Security Report`.
...
- SSRF via the Unraid server URL configuration
- Injection in VM names, share names, or container names passed to the Unraid API
- Privilege escalation via unintended Unraid management operations
- Information disclosure through system status responses exposing topology or credentials
- Dependency vulnerabilities with a plausible exploitation path in unraid-mcp-server's usage
